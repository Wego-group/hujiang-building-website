...
import { validateSeo } from "../lib/news-seo.mjs";
...
function validateBasic(article) {
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(article.slug ?? "")) throw new Error("Invalid slug (must be lowercase-hyphenated).");
...
for (const dateEntry of await readdir(queueRoot, { withFileTypes: true }).catch(() => [])) {
  ...
  for (const item of await readdir(dated, { withFileTypes: true })) {
    ...
    try {
      const article = parse(await readFile(path.join(source, "article.md"), "utf8"));
      validateBasic(article);
      const seo = validateSeo(article);
      if (!seo.valid) {
        throw new Error(`SEO validation failed (${seo.errors.length} error(s)): ${seo.errors.map(e => e.message).join("; ")}`);
      }
      const destination = path.join(root, "content", "news", article.slug);
      await mkdir(destination, { recursive: true });
      await writeFile(path.join(destination, "article.json"), `${JSON.stringify(article, null, 2)}\n`);
      if (article.cover) {
        const target = path.join(root, "public", "images", "news", article.slug);
        await mkdir(target, { recursive: true });
        await cp(path.join(source, article.cover), path.join(target, article.cover));
      }
      const archive = path.join(root, "news-published", dateEntry.name, item.name);
      await mkdir(path.dirname(archive), { recursive: true });
      await rename(source, archive);
      console.log(`Published ${article.slug} (SEO: ${seo.score}/100)`);
      publishedCount++;
    } catch (error) {
      const failed = path.join(root, "news-failed", dateEntry.name, item.name);
